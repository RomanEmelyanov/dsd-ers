...
/* for linker add two lines below to the end of the file '/CMakeLists.txt':
find_package(OpenSSL REQUIRED)
target_link_libraries(dsd-fme OpenSSL::SSL)
*/

void DES_OFB(uint8_t iv[], uint8_t key[], uint8_t cipher[], uint8_t plain[], uint8_t plain_size) {
  int outlen1, outlen2;
  ...
  if (legacy == NULL) {
    printf("Failed to load Legacy provider\nEVP_des_ofb is legacy for OpenSSL > 3");
    ERR_print_errors_fp(stderr);
  }

  ctx = EVP_CIPHER_CTX_new();
  EVP_EncryptInit(ctx, EVP_des_ofb(), key, iv);
  EVP_EncryptUpdate(ctx, cipher, &outlen1, plain, plain_size);
  EVP_EncryptFinal(ctx, cipher + outlen1, &outlen2);
  EVP_CIPHER_CTX_free(ctx);
}

void AES128_OFB(uint8_t iv[], uint8_t key[], uint8_t cipher[], uint8_t plain[], uint8_t plain_size) {
  int outlen1, outlen2;
  EVP_CIPHER_CTX *ctx = NULL;
  ctx = EVP_CIPHER_CTX_new();
...
void AES192_OFB(uint8_t iv[], uint8_t key[], uint8_t cipher[], uint8_t plain[], uint8_t plain_size) {
  int outlen1, outlen2;
  EVP_CIPHER_CTX *ctx = NULL;
  ctx = EVP_CIPHER_CTX_new();
...
void AES256_OFB(uint8_t iv[], uint8_t key[], uint8_t cipher[], uint8_t plain[], uint8_t plain_size) {
  int outlen1, outlen2;
  EVP_CIPHER_CTX *ctx = NULL;
  ctx = EVP_CIPHER_CTX_new();
...
// START SMS PATCH
  ...
  /* this vars in dsd.h must include to state struct:
  uint8_t sms_cipher_type;
  uint8_t des_key[8];
  uint8_t des_iv[8];
  uint8_t aes_key[32];
  uint8_t aes_iv[32];
  uint8_t des_plain[8+12*3]; we will parse three frame maximum
  uint8_t des_cipher[8+12*3];
  uint8_t aes_plain[32+12*3];
  uint8_t aes_cipher[32+12*3];
  */
  unsigned long long int lfsr = 0;
  int o;

  // Slot 1 Data Header - Proprietary Packet Data - P_SAP 10 [Short DT] - MFID 10 [Motorola] WITHOUT FEC ERROR!
  if ( strcmp (state->fsubtype, " DATA") == 0)
  {
    if (opts->payload == 1) {
      fprintf (stderr, "\nDMR SMS HEADER: ");
      for (i = 0; i < pdu_len; i++) { 
    ...
    lfsr = DMR_PDU[6] << 24 | DMR_PDU[7] << 16 | DMR_PDU[8] << 8 | DMR_PDU[9];
    
    if (opts->payload == 1) fprintf (stderr, "LFSR: %016llX\n", lfsr);
    ...
    if (opts->payload == 1) fprintf (stderr, "IV  DES64: %016llX\n", lfsr);

    state->sms_cipher_type = DMR_PDU[2];
    if ( state->sms_cipher_type == 0x21 ) { state->pos = 8; } // DES
    if ( state->sms_cipher_type == 0x41 ) { state->pos = 16; } // AES128
    if ( state->sms_cipher_type == 0x31 ) { state->pos = 16; } // AES192
    if ( state->sms_cipher_type == 0x51 ) { state->pos = 16; } // AES256

    state->des_iv[0] = ((lfsr & 0xFF00000000000000) >> 56);
    state->des_iv[1] = ((lfsr & 0xFF000000000000) >> 48);
    state->des_iv[2] = ((lfsr & 0xFF0000000000) >> 40);
    state->des_iv[3] = ((lfsr & 0xFF00000000) >> 32);
    state->des_iv[4] = ((lfsr & 0xFF000000) >> 24);
    state->des_iv[5] = ((lfsr & 0xFF0000) >> 16);
    state->des_iv[6] = ((lfsr & 0xFF00) >> 8);
    state->des_iv[7] = ((lfsr & 0xFF) >> 0);

    state->aes_iv[0] = ((lfsr & 0xFF00000000000000) >> 56);
    state->aes_iv[1] = ((lfsr & 0xFF000000000000) >> 48);
    state->aes_iv[2] = ((lfsr & 0xFF0000000000) >> 40);
    state->aes_iv[3] = ((lfsr & 0xFF00000000) >> 32);
    state->aes_iv[4] = ((lfsr & 0xFF000000) >> 24);
    state->aes_iv[5] = ((lfsr & 0xFF0000) >> 16);
    state->aes_iv[6] = ((lfsr & 0xFF00) >> 8);
    state->aes_iv[7] = ((lfsr & 0xFF) >> 0);
    ...
    state->aes_iv[8]  = ((lfsr & 0xFF00000000000000) >> 56);
    state->aes_iv[9]  = ((lfsr & 0xFF000000000000) >> 48);
    state->aes_iv[10] = ((lfsr & 0xFF0000000000) >> 40);
    state->aes_iv[11] = ((lfsr & 0xFF00000000) >> 32);
    state->aes_iv[12] = ((lfsr & 0xFF000000) >> 24);
    state->aes_iv[13] = ((lfsr & 0xFF0000) >> 16);
    state->aes_iv[14] = ((lfsr & 0xFF00) >> 8);
    state->aes_iv[15] = ((lfsr & 0xFF) >> 0);

    if (opts->payload == 1) fprintf (stderr, "IV AES128: %016llX\n", lfsr);
    ...
    state->aes_iv[16] = ((lfsr & 0xFF00000000000000) >> 56);
    state->aes_iv[17] = ((lfsr & 0xFF000000000000) >> 48);
    state->aes_iv[18] = ((lfsr & 0xFF0000000000) >> 40);
    state->aes_iv[19] = ((lfsr & 0xFF00000000) >> 32);
    state->aes_iv[20] = ((lfsr & 0xFF000000) >> 24);
    state->aes_iv[21] = ((lfsr & 0xFF0000) >> 16);
    state->aes_iv[22] = ((lfsr & 0xFF00) >> 8);
    state->aes_iv[23] = ((lfsr & 0xFF) >> 0);

    if (opts->payload == 1) fprintf (stderr, "IV AES192: %016llX\n", lfsr);
    ...
    state->aes_iv[24] = ((lfsr & 0xFF00000000000000) >> 56);
    state->aes_iv[25] = ((lfsr & 0xFF000000000000) >> 48);
    state->aes_iv[26] = ((lfsr & 0xFF0000000000) >> 40);
    state->aes_iv[27] = ((lfsr & 0xFF00000000) >> 32);
    state->aes_iv[28] = ((lfsr & 0xFF000000) >> 24);
    state->aes_iv[29] = ((lfsr & 0xFF0000) >> 16);
    state->aes_iv[30] = ((lfsr & 0xFF00) >> 8);
    state->aes_iv[31] = ((lfsr & 0xFF) >> 0);

    if (opts->payload == 1) fprintf (stderr, "IV AES256: %016llX\n", lfsr);

    state->des_key[0] = ((state->R & 0xFF00000000000000) >> 56);
    state->des_key[1] = ((state->R & 0xFF000000000000) >> 48);
    state->des_key[2] = ((state->R & 0xFF0000000000) >> 40);
    state->des_key[3] = ((state->R & 0xFF00000000) >> 32);
    state->des_key[4] = ((state->R & 0xFF000000) >> 24);
    state->des_key[5] = ((state->R & 0xFF0000) >> 16);
    state->des_key[6] = ((state->R & 0xFF00) >> 8);
    state->des_key[7] = ((state->R & 0xFF) >> 0);
    
    state->aes_key[0]  = ((state->K1 & 0xFF00000000000000) >> 56);
    state->aes_key[1]  = ((state->K1 & 0xFF000000000000) >> 48);
    state->aes_key[2]  = ((state->K1 & 0xFF0000000000) >> 40);
    state->aes_key[3]  = ((state->K1 & 0xFF00000000) >> 32);
    state->aes_key[4]  = ((state->K1 & 0xFF000000) >> 24);
    state->aes_key[5]  = ((state->K1 & 0xFF0000) >> 16);
    state->aes_key[6]  = ((state->K1 & 0xFF00) >> 8);
    state->aes_key[7]  = ((state->K1 & 0xFF) >> 0);

    state->aes_key[8]  = ((state->K2 & 0xFF00000000000000) >> 56);
    state->aes_key[9]  = ((state->K2 & 0xFF000000000000) >> 48);
    state->aes_key[10] = ((state->K2 & 0xFF0000000000) >> 40);
    state->aes_key[11] = ((state->K2 & 0xFF00000000) >> 32);
    state->aes_key[12] = ((state->K2 & 0xFF000000) >> 24);
    state->aes_key[13] = ((state->K2 & 0xFF0000) >> 16);
    state->aes_key[14] = ((state->K2 & 0xFF00) >> 8);
    state->aes_key[15] = ((state->K2 & 0xFF) >> 0);

    state->aes_key[16] = ((state->K3 & 0xFF00000000000000) >> 56);
    state->aes_key[17] = ((state->K3 & 0xFF000000000000) >> 48);
    state->aes_key[18] = ((state->K3 & 0xFF0000000000) >> 40);
    state->aes_key[19] = ((state->K3 & 0xFF00000000) >> 32);
    state->aes_key[20] = ((state->K3 & 0xFF000000) >> 24);
    state->aes_key[21] = ((state->K3 & 0xFF0000) >> 16);
    state->aes_key[22] = ((state->K3 & 0xFF00) >> 8);
    state->aes_key[23] = ((state->K3 & 0xFF) >> 0);

    state->aes_key[24] = ((state->K4 & 0xFF00000000000000) >> 56);
    state->aes_key[25] = ((state->K4 & 0xFF000000000000) >> 48);
    state->aes_key[26] = ((state->K4 & 0xFF0000000000) >> 40);
    state->aes_key[27] = ((state->K4 & 0xFF00000000) >> 32);
    state->aes_key[28] = ((state->K4 & 0xFF000000) >> 24);
    state->aes_key[29] = ((state->K4 & 0xFF0000) >> 16);
    state->aes_key[30] = ((state->K4 & 0xFF00) >> 8);
    state->aes_key[31] = ((state->K4 & 0xFF) >> 0);
}

  if ( strcmp (state->fsubtype, " R12U ") == 0 && state->sms_cipher_type == 0x21 )
  {
    if (opts->payload == 1) fprintf (stderr, "\nDMR SMS DES64 ENCRYPTED MESSAGE: ");
    for (i = 0; i < pdu_len; i++) {
      state->des_cipher[state->pos+i] = DMR_PDU[i];
      if (opts->payload == 1) fprintf (stderr, "%02X", DMR_PDU[i]);
    };

    DES_OFB(state->des_iv, state->des_key, state->des_plain, state->des_cipher, sizeof(state->des_plain));

    if (opts->payload == 1) {
      fprintf(stderr, "\ncipher: ");
      for (o = 0; o < sizeof(state->des_cipher); o++) { fprintf (stderr, "%02X", state->des_cipher[o]); }
      fprintf(stderr, "\n   key: ");
      for (o = 0; o < sizeof(state->des_key); o++) { fprintf (stderr, "%02X", state->des_key[o]); }
      fprintf(stderr, "\n    iv: ");
      for (o = 0; o < sizeof(state->des_iv); o++) { fprintf (stderr, "%02X", state->des_iv[o]); }
      fprintf(stderr, "\n plain: ");
      for (o = 0; o < sizeof(state->des_plain); o++) { fprintf (stderr, "%02X", state->des_plain[o]); }
      fprintf(stderr, "\n");
    }

    o = state->pos;
    fprintf(stderr, "\nDMR SMS DES64 DECRYPTED TEXT: %c%c%c%c%c%c\n", 
      state->des_plain[o+1], state->des_plain[o+3], state->des_plain[o+5], 
      state->des_plain[o+7], state->des_plain[o+9], state->des_plain[o+11]);
  
    if (opts->payload == 1) fprintf (stderr, "\nPosition: %d", state->pos);
    state->pos += 12;
  }

  if ( strcmp (state->fsubtype, " R12U ") == 0 && state->sms_cipher_type == 0x41)
  {
    fprintf (stderr, "\nDMR SMS AES128 ENCRYPTED MESSAGE: ");
    for (i = 0; i < pdu_len; i++) {
     state->aes_cipher[state->pos+i] = DMR_PDU[i];
     if (opts->payload == 1) fprintf (stderr, "%02X", DMR_PDU[i]);
    };

    AES128_OFB(state->aes_iv, state->aes_key, state->aes_plain, state->aes_cipher, sizeof(state->aes_cipher));
    
    if (opts->payload == 1) {
      fprintf(stderr, "\ncipher: ");
      for (o = 0; o < sizeof(state->aes_cipher); o++) { fprintf (stderr, "%02X", state->aes_cipher[o]); }
      fprintf(stderr, "\n   key: ");
      for (o = 0; o < sizeof(state->aes_key); o++) { fprintf (stderr, "%02X", state->aes_key[o]); }
      fprintf(stderr, "\n    iv: ");
      for (o = 0; o < sizeof(state->aes_iv); o++) { fprintf (stderr, "%02X", state->aes_iv[o]); }
      fprintf(stderr, "\n plain: ");
      for (o = 0; o < sizeof(state->aes_plain); o++) { fprintf (stderr, "%02X", state->aes_plain[o]); }
      fprintf(stderr, "\n");
    }

    o = state->pos;

    fprintf(stderr, "\nDMR SMS AES128 DECRYPTED TEXT: %c%c%c%c%c%c\n", 
      state->aes_plain[o+1], state->aes_plain[o+3], state->aes_plain[o+5], 
      state->aes_plain[o+7], state->aes_plain[o+9], state->aes_plain[o+11]);

    if (opts->payload == 1) fprintf (stderr, "\nPosition: %d", state->pos);
    state->pos += 12;
  }

  if ( strcmp (state->fsubtype, " R12U ") == 0 && state->sms_cipher_type == 0x31)
  {
    fprintf (stderr, "\nDMR SMS AES192 ENCRYPTED MESSAGE: ");
    for (i = 0; i < pdu_len; i++) {
     state->aes_cipher[state->pos+i] = DMR_PDU[i];
     if (opts->payload == 1) fprintf (stderr, "%02X", DMR_PDU[i]);
    };

    AES192_OFB(state->aes_iv, state->aes_key, state->aes_plain, state->aes_cipher, sizeof(state->aes_cipher));
    ...
    o = state->pos;
    fprintf(stderr, "\nDMR SMS AES192 DECRYPTED TEXT: %c%c%c%c%c%c\n", 
      state->aes_plain[o+1], state->aes_plain[o+3], state->aes_plain[o+5], 
      state->aes_plain[o+7], state->aes_plain[o+9], state->aes_plain[o+11]);
  ...
  if ( strcmp (state->fsubtype, " R12U ") == 0 && state->sms_cipher_type == 0x51)
  {
    fprintf (stderr, "\nDMR SMS AES256 ENCRYPTED MESSAGE: ");
    for (i = 0; i < pdu_len; i++) {
     state->aes_cipher[state->pos+i] = DMR_PDU[i];
     if (opts->payload == 1) fprintf (stderr, "%02X", DMR_PDU[i]);
    };

    AES256_OFB(state->aes_iv, state->aes_key, state->aes_plain, state->aes_cipher, sizeof(state->aes_cipher));
    ...
    o = state->pos;
    fprintf(stderr, "\nDMR SMS AES256 DECRYPTED TEXT: %c%c%c%c%c%c\n", 
      state->aes_plain[o+1], state->aes_plain[o+3], state->aes_plain[o+5], 
      state->aes_plain[o+7], state->aes_plain[o+9], state->aes_plain[o+11]);
